...
export default function NewsForm({ onCreate, onUpdate, isEdit, editedValues, UpdateImage }: { onCreate: any, onUpdate: any, isEdit: boolean, editedValues: any, UpdateImage: any }) {
...
const categoriesList=['law','sport','politics','health','Business and economy','Miscellaneous']
  const [editorState, setEditorState] = useState(() => {
    if (isEdit) {
      const contentState = convertFromHTML(DOMPurify.sanitize(editedValues?.newsContent || ''));
  ...
  const [formData, setFormData] = useState({
    newsTitle: isEdit ? editedValues.newsTitle:"",
    // newsContent: selectedNews?.newsContent || "",
     newsLang:isEdit ? editedValues.newsLang : "",
      isFlag: isEdit ? editedValues.isFlag : "",
    newsCategory:isEdit?editedValues.newsCategory:""
  });
  ...
  const handleUpdateImage = async () => {
    await UpdateImage(editedValues.newsId);
  };
                ...
                <div className="relative z-20 bg-white dark:bg-strokedark ">
                  ...
                  <select
                    ...
                    value={(isEdit && status == '') ? editedValues.isFlag : status}
                                      ...
                                      name="isFlag"
                  >
                    <option value={"not ready"}>{t('no')}</option>
                    <option value={"ready"}>{t('yes')}</option>
                  </select>
